class TerraspaceBundler::Exporter
  class Copy < Base
    ...
    def mod
      ...
      logger.debug "cp -r #{src_path} #{mod_path}"
    ...
    def src_path
      path = stage_path(@mod.copy_source_path) # from PathConcern
      puts "src_path path #{path}".color(:yellow)
      puts "@mod.subfolder #{@mod.subfolder}"
